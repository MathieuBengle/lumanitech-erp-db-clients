#!/bin/bash
# Deploys the clients database while reusing mysql_config_editor login-path credentials.
...
# Defaults
DB_NAME="lumanitech_erp_clients"
DB_HOST="localhost"
DB_USER="admin"
LOGIN_PATH="local"
WITH_SEEDS=false
...
Deploy the clients database schema, migrations, and optional seed data.

Options:
  -d, --database NAME     Database name (default: lumanitech_erp_clients)
...
Examples:
  ...
  ./scripts/deploy.sh --login-path=local --database=lumanitech_projects
EOF
}
...
main() {
    info "Starting Clients DB deployment"
    ...
    apply_migrations
    load_seed_data
    info "Clients database deployment complete"
}
